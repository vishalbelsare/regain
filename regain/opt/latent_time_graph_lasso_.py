...
def prox_trace_indicator(a, lamda):
    ...
    xi = tf.maximum(es - lamda, 0)
    return tf.matmul(tf.matmul(Q, tf.diag(xi)), Q, transpose_b=True)

def gpu_prox(a, lamda):
    with tf.device("/gpu:0"):
        X = prox_trace_indicator(a, lamda)
...
def latent_time_graph_lasso(
    ...
    for iteration_ in range(max_iter):
        ...
        W_0 = np.array([gpu_prox(a, lamda=tau / (rho * div))
